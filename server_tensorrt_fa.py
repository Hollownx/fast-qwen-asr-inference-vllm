...
# 对齐模型配置（forced_alignment 时间戳功能）
ALIGNER_MODEL_NAME = os.getenv("ALIGNER_MODEL_NAME", "Qwen/Qwen3-ForcedAligner-0.6B")
ENABLE_ALIGNER = get_env_bool("ENABLE_ALIGNER", "true")
...
# UE RMS 阈值过滤配置
UE_ENABLE_THRESHOLD_FILTER = get_env_bool("UE_ENABLE_THRESHOLD_FILTER", "false")
...
aligner: Any = None            # 强制对齐模型（Qwen3ForcedAligner），可选
...
async def load_model_background():
    """后台加载模型"""
    global model, aligner, model_status
    ...
    # 加载对齐模型（forced_alignment 时间戳功能）
    if ENABLE_ALIGNER:
        try:
            from qwen_asr import Qwen3ForcedAligner
            aligner_source, aligner_hf_kwargs = resolve_hf_model_source(ALIGNER_MODEL_NAME)
            logger.info("加载对齐模型: requested=%s resolved=%s", ALIGNER_MODEL_NAME, aligner_source)
            aligner = await asyncio.to_thread(
            ...
            logger.warning(f"对齐模型加载失败（forced_alignment 将不可用）: {e}")
            aligner = None
    else:
        logger.info("对齐模型已禁用（ENABLE_ALIGNER=false）")
...
async def transcribe(
    ...
    forced_alignment: bool = Query(False, description="If true, return word-level timestamps."),
):
    ...
    try:
        ...
        if forced_alignment:
            if aligner is None:
                raise HTTPException(status_code=503, detail="Aligner model not loaded. Set ENABLE_ALIGNER=true.")
...
async def websocket_streaming(
    ...
    forced_alignment: bool = Query(False),  # API 对称性保留，流式暂不支持
):
    """
    WebSocket 流式转录
    ...
    forced_alignment: 流式模式暂不支持，仅 /transcribe 批量接口可用。
    """
    await ws.accept()
